...
void FActionCommandHandler::RegisterCommands()
{
	...
	Cmd = FDispatcherDelegate::CreateRaw(this, &FActionCommandHandler::StartRec);
	Help = "Start Rec -cameraTag -imageType -filepath -deltaTime -width -height -imageNumber";
	CommandDispatcher->BindCommand("vset /action/rec/start [str]", Cmd, Help);
	CommandDispatcher->BindCommand("vset /action/rec/start [str] [str]", Cmd, Help);
	CommandDispatcher->BindCommand("vset /action/rec/start [str] [str] [str]", Cmd, Help);
	CommandDispatcher->BindCommand("vset /action/rec/start [str] [str] [str] [str]", Cmd, Help);
	CommandDispatcher->BindCommand("vset /action/rec/start [str] [str] [str] [str] [str]", Cmd, Help);
	CommandDispatcher->BindCommand("vset /action/rec/start [str] [str] [str] [str] [str] [str]", Cmd, Help);
	CommandDispatcher->BindCommand("vset /action/rec/start [str] [str] [str] [str] [str] [str] [str]", Cmd, Help);

	Cmd = FDispatcherDelegate::CreateRaw(this, &FActionCommandHandler::StopRec);
	Help = "Stop Rec -cameraTag";
	CommandDispatcher->BindCommand("vset /action/rec/stop [str]", Cmd, Help);
...
FExecStatus FActionCommandHandler::StartRec(const TArray<FString>& Args)
{
	FRecordingThread::startRecording(Args);

	return FExecStatus::OK();
}


FExecStatus FActionCommandHandler::StopRec(const TArray<FString>& Args)
{
